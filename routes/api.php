...
use App\Http\Controllers\Admin\DisbursementOrderController;
...
Route::prefix('admin')->name('admin.')->middleware(['installed', 'apiKey', 'localization'])->group(function () {
    ...
    Route::prefix('setting')->name('setting.')->group(function () {
        ...
        Route::prefix('integrations')->name('integrations.')->group(function () {
            ...
            Route::post('/integrationAPI', [IntegrationController::class, 'SaveIntegrationAPI']);
            Route::get('/', [IntegrationController::class, 'index']);
            Route::get('/listsWithAPISetting', [IntegrationController::class, 'listsWithAPISetting']);
            Route::get('/show/{integration}', [IntegrationController::class, 'show']);
        ...
        Route::prefix('KitchenPrinterSetting')->name('KitchenPrinterSetting.')->group(function () {
            Route::get('/', [KitchenPrinterController::class, 'index']);
            Route::get('/show/{KitchenPrinterSetting}', [KitchenPrinterController::class, 'show']);
            Route::post('/', [KitchenPrinterController::class, 'store']);
            Route::match(['put', 'patch'], '/{KitchenPrinterSetting}', [KitchenPrinterController::class, 'update']);
            Route::post('/testprint/{KitchenPrinterSetting}', [KitchenPrinterController::class, 'testprint']);
            Route::delete('/{KitchenPrinterSetting}', [KitchenPrinterController::class, 'destroy']);
        });
        ...
        Route::prefix('Shift')->name('Shift.')->group(function () {
            ...
            Route::get('/show/{Shift}', [ShiftController::class, 'show']);
            Route::post('/', [ShiftController::class, 'store']);
            Route::match(['put', 'patch'], '/{Shift}', [ShiftController::class, 'update']);
            Route::delete('/{Shift}', [ShiftController::class, 'destroy']);
        });
        Route::prefix('PosDevice')->name('PosDevice.')->group(function () {
            Route::get('/', [PosDeviceController::class, 'index']);
            Route::get('/show/{PosDevice}', [PosDeviceController::class, 'show']);
            Route::post('/', [PosDeviceController::class, 'store']);
            Route::match(['put', 'patch'], '/{PosDevice}', [PosDeviceController::class, 'update']);
            Route::delete('/{PosDevice}', [PosDeviceController::class, 'destroy']);
        });
    ...
    Route::prefix('pos')->name('pos.')->group(function () {
        ...
        // Route::get('sessions', [\App\Http\Controllers\Admin\POSSessionController::class, 'apiIndex']);
        // Route::post('sessions', [\App\Http\Controllers\Admin\POSSessionController::class, 'apiStore']);
        // Route::get('active-session', [\App\Http\Controllers\Admin\POSSessionController::class, 'checkActiveSession']);
        // Route::get('sessions/active', [\App\Http\Controllers\Admin\POSSessionController::class, 'checkActiveSession']);
        // Route::get('dashboard-stats', [\App\Http\Controllers\Admin\POSSessionController::class, 'getDashboardStats']);

        // // Cash movements
        // Route::post('sessions/{sessionId}/cash-movements', [\App\Http\Controllers\Admin\POSSessionController::class, 'apiRecordMovement']);
        // Route::post('sessions/{sessionId}/cash-movement', [\App\Http\Controllers\Admin\POSSessionController::class, 'apiRecordMovement']);
        // Route::get('sessions/{sessionId}/cash-movements', [\App\Http\Controllers\Admin\POSSessionController::class, 'apiCashMovements']);
        // Route::get('sessions/{sessionId}/movements', [\App\Http\Controllers\Admin\POSSessionController::class, 'apiCashMovements']);

        // // Shift types
        // Route::get('shift-types', [\App\Http\Controllers\Admin\ShiftTypeController::class, 'apiIndex']);

        // // Approvals
        // Route::get('approvals', [\App\Http\Controllers\Admin\POSSessionController::class, 'apiApprovalsIndex']);

        // // Reports
        // Route::get('sessions/report', [\App\Http\Controllers\Admin\POSSessionController::class, 'apiReport']);
    });
    ...
    Route::prefix('pos-shifts')->name('posshifts.')->group(function () {
        Route::get('/', [PosShiftController::class, 'index']);
        Route::post('/', [PosShiftController::class, 'store'])->name('posshift.store');
        Route::match(['post', 'put', 'patch'], '/{posshift}', [PosShiftController::class, 'CloseSessionShift']);
        Route::match(['post', 'put', 'patch'], '/{posshift}', [PosShiftController::class, 'ConfirmSessionShift']);
        Route::get('/generate-session/{device}', [PosShiftController::class, 'generateSessionNumber'])->name('BuildSessionNumber');
    });
    ...
    Route::prefix('auditLog')->name('auditLog.')->group(function () {
    ...
    Route::prefix('operation-logs')->name('operation-logs.')->group(function () {
        Route::post('/log-action', function (Request $request) {
            ...
            OperationLog::create([
                ...
                'description' => $data['description'] ?? 'User attempted action',
        ...
        Route::get('/', function (Request $request) {
            // Gate::authorize('viewAny', OperationLog::class);
        ...
        Route::put('/mark-seen', function () {
            OperationLog::where('seen', false)->update(['seen' => true]);
            return response()->json(['message' => 'تم تعليم جميع العمليات كمقروءة']);
        });
    });

    Route::prefix('PurchaseInvoiceForm')->name('PurchaseInvoiceForm.')->group(function () {
        Route::get('/', [PurchaseInvoiceController::class, 'index']);
        Route::post('/', [PurchaseInvoiceController::class, 'store']);
    });
    Route::prefix('OpeningBalanceForm')->name('OpeningBalanceForm.')->group(function () {
        Route::get('/', [OpeningBalanceController::class, 'index']);
        Route::post('/', [OpeningBalanceController::class, 'store']);
    });
    Route::prefix('DisbursementOrderForm')->name('DisbursementOrderForm.')->group(function () {
        Route::get('/', [DisbursementOrderController::class, 'index']);
        Route::post('/', [DisbursementOrderController::class, 'store']);
    });
    Route::prefix('SupplyOrderForm')->name('SupplyOrderForm.')->group(function () {
        Route::get('/', [SupplyOrderController::class, 'index']);
        Route::post('/', [SupplyOrderController::class, 'store']);
    });
    Route::prefix('InventoryReport')->name('InventoryReport.')->group(function () {
        Route::get('/stock-balances', [ReportController::class, 'stockBalances']);
        Route::get('/movement-history', [ReportController::class, 'movementHistory']);
    });
...
Route::prefix('pos')->name('admin.pos.')->middleware(['installed', 'auth:sanctum'])->group(function () {
    // Session management
    Route::get('sessions', [\App\Http\Controllers\Admin\POSSessionController::class, 'apiIndex']);
    Route::post('sessions', [\App\Http\Controllers\Admin\POSSessionController::class, 'apiStore']);
    Route::get('active-session', [\App\Http\Controllers\Admin\POSSessionController::class, 'checkActiveSession']);
