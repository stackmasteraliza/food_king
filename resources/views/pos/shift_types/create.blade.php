...
@section('content')
    <div class="container">
        ...
        <form action="{{ route('shift-types.store') }}" method="POST">
            ...
            <div class="form-group">
                ...
                <select name="parent_shift_id" id="parent_shift_id" class="form-control @error('parent_shift_id') is-invalid @enderror">
                    ...
                    @foreach($parentShifts as $shift)
                        <option value="{{ $shift->id }}" {{ old('parent_shift_id') == $shift->id ? 'selected' : '' }}>{{ $shift->name }}</option>
                    @endforeach
                </select>
            ...
            <button type="submit" class="btn btn-primary mt-3">Create Shift Type</button>
        </form>
    </div>
@endsection
