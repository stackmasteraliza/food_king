  ...
  <div id="modal" class="modal">
    <div class="modal-dialog">
      <div class="modal-header">
        <h3 class="modal-title">{{ $t("menu.shifts") }}</h3>
        <button
...
export default {
  ...
  computed: {
    addButton: function () {
      return { title: this.$t("button.add_Shift") };
    },
  },
  methods: {
    reset: function () {
      appService.modalHide();
      this.$store.dispatch("Shift/reset").then().catch();
      this.errors = {};
    ...
    save: function () {
      try {
        const tempId = this.$store.getters["Shift/temp"].temp_id;
        this.loading.isActive = true;
        this.$store
          .dispatch("Shift/save", this.props)
          .then((res) => {
            appService.modalHide();
