    ...
    <div class="db-card">
      <div class="db-card-header border-none">
        ...
        <div class="db-card-filter">
          ...
          />
          <FilterComponent
            @click.prevent="handleSlide('online-order-filter')"
          />
          ...
          <tbody class="db-table-body" v-if="orders.length > 0">
            <tr class="db-table-body-tr" v-for="order in orders" :key="order">
              ...
              <td class="db-table-body-td">
                ...
                <span
                  :class="orderStatusClass(order.is_advance_order)"
...
export default {
  ...
  methods: {
    ...
    clear: function () {
      ...
      this.props.search.excepts =
        orderTypeEnum.POS + "|" + orderTypeEnum.DINING_TABLE;
    ...
    pdf: function () {
      this.loading.isActive = true;
      this.$store
        .dispatch("onlineOrder/pdf", this.props.search)
        .then((res) => {
          this.loading.isActive = false;
          const blob = new Blob([res.data]);
          const link = document.createElement("a");
