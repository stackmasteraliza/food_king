<template>
  ...
  >
    <div
      class="max-w-sm w-full h-screen absolute top-0 right-0 translate-x-full bg-white transition"
    >
      <div
        :class="
          carts.length === 0
            ? 'flex items-center justify-center flex-col text-center overflow-y-auto'
            : 'thin-scrolling'
        "
        class="h-[calc(100vh-250px)] lg:h-[calc(100vh-220px)] p-4 relative"
      >
        <h3
          :class="carts.length === 0 ? 'mb-16' : 'mb-5'"
          class="text-xl font-semibold capitalize text-center"
        >
          {{ $t("label.my_cart") }}
        </h3>
        <button
          @click.prevent="closeCanvas('cart')"
          class="fa-solid fa-xmark absolute top-2 right-3 text-white bg-[#FB4E4E] xmark-btn"
        ></button>
          ...
          >
            <div class="flex items-center gap-3 mb-2">
              ...
              <div class="w-full">
                ...
                <p
                  v-if="
                    Object.keys(cart.item_variations.variations).length !== 0
                  "
                  class="capitalize text-xs mb-1.5"
                >
                  <span
                    v-for="(variation, variationName) in cart.item_variations
                      .names"
                  >
                ...
                <div class="flex items-center justify-between gap-2">
                  <h3 style="font-family: 'saudi_riyal'" class="text-xs font-semibold">
                    {{
                      currencyFormat(
                        cart.total,
                  ...
                  <div class="flex items-center indec-group">
                    <button
                      @click.prevent="quantityDecrement(index)"
                    ...
                    <input
                      ...
                      v-on:keyup="quantityUp(index, $event)"
                      type="number"
                      :value="cart.quantity"
      ...
      <div v-if="carts.length > 0" class="p-4">
        <div
          class="flex items-center justify-between gap-2 rounded-xl p-3 mb-3 border border-gray-1"
        >
          <h3 class="capitalize text-sm font-medium">
            {{ $t("label.subtotal") }}
          </h3>
          <h4 style="font-family: 'saudi_riyal'" class="text-sm font-medium text-[#1AB759]">
            {{
              currencyFormat(
                subtotal,
                setting.site_digit_after_decimal_point,
...
export default {
  ...
  methods: {
    ...
    quantityUp: function (id, e) {
      if (e.target.value > 0) {
        this.$store
          .dispatch("tableCart/quantity", { id: id, status: e.target.value })
          .then()
          .catch();
      }
    },
    quantityIncrement: function (id) {
      this.$store
        .dispatch("tableCart/quantity", { id: id, status: "increment" })
        .then()
        .catch();
    },
    quantityDecrement: function (id) {
      this.$store
        .dispatch("tableCart/quantity", { id: id, status: "decrement" })
        .then()
        .catch();
    },
