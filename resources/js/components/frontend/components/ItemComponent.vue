<template>
    ...
    >
      ...
      <div class="product-card-list-content-group">
        <div class="product-card-list-header-group">
          ...
          <button
            class="product-card-list-info-btn info-btn"
            data-modal="#item-info-modal"
            @click.prevent="infoModalShow(item.name, item.caution)"
          >
            <i
              class="lab lab-information font-fill-paragraph transition lab-font-size-16"
            ></i>
          </button>
        </div>
        ...
        <div class="product-card-list-footer-group">
          <div class="product-card-list-price-group">
            <del
              v-if="item.offer.length > 0"
              style="font-family: 'saudi_riyal'"
              class="product-card-grid-price-previous"
            >
              {{ item.currency_price }} &#xE900;
            </del>
            <h4
              style="font-family: 'saudi_riyal'"
              class="product-card-list-price-current"
            >
              {{
                item.offer.length > 0
  ...
  <div
    v-else-if="design === itemDesignEnum.GRID"
    ...
    >
      ...
      <div class="product-card-grid-content-group">
        <div class="product-card-grid-header-group">
          ...
          <button
            type="button"
            class="product-card-grid-info-btn info-btn"
            data-modal="#item-info-modal"
            @click.prevent="infoModalShow(item.name, item.caution)"
          >
            <i
        ...
        <div class="product-card-grid-footer-group">
          <div class="product-card-grid-price-group">
            ...
            <h4
              style="font-family: 'saudi_riyal'"
              class="product-card-grid-price-current"
            >
              {{
                item.offer.length > 0
                  ? item.offer[0].currency_price
  ...
  <div
    id="item-info-modal"
    ref="itemInfoModal"
    class="modal ff-modal info-modal"
  >
    ...
    <div class="modal-dialog max-w-[647px]" v-if="item">
      <div class="modal-header items-start border-none pb-0">
        <div class="flex gap-4">
          <img
            class="flex-shrink-0 w-[72px] h-[72px] object-cover rounded-lg"
            :src="item.thumb"
            alt="thumbnail"
          />
          <div class="flex-auto">
        ...
        <button
          class="modal-close lab-close-circle-line font-fill-danger lab-font-size-24"
      ...
      <div class="modal-body">
        <div class="flex items-center gap-2 mb-4">
          <h3
            class="text-sm leading-6 font-medium first-letter:uppercase text-heading"
          >
            {{ $t("label.quantity") }}:
          </h3>
          <div
            class="flex items-center indec-group py-1 px-2 rounded-xl bg-[#F7F7FC]"
          >
            <button
        ...
        <div class="mb-4" v-if="item.itemAttributes.length > 1">
          <div class="row">
            <div
              v-for="itemAttribute in item.itemAttributes"
              class="col-12 sm:col-6"
            >
              <label
                class="text-sm leading-6 block font-medium capitalize mb-1.5 text-heading"
              >
                {{ itemAttribute.name }}
              </label>
              <div class="relative">
                ...
                <select
                  @change.prevent="
                    changeVariationAdjust(
                      itemAttribute.id,
                      temp.item_variations.variations[itemAttribute.id]
                    )
                  "
                ...
                >
                  <option
                    :value="variation.id"
                    v-for="variation in item.variations[itemAttribute.id]"
                    :key="variation"
                  >
        ...
        <div class="mb-4" v-else-if="item.itemAttributes.length > 0">
          <h3
            class="text-sm leading-6 font-medium capitalize mb-2 text-heading"
          >
                  ...
                  >
                    <div class="custom-checkbox w-3 h-3">
                      <input
                        ...
                        @change.prevent="
                          changeExtra($event, extra.id, extra.name)
                        "
                        :value="extra.id"
                        type="checkbox"
                      ...
                      />
                      <i
                        class="fa-solid fa-check custom-checkbox-icon leading-[9px] text-[9px] rounded-[3px]"
                      ></i>
                    </div>
                    <div>
            ...
            <Swiper :speed="1000" slidesPerView="auto" :spaceBetween="16">
              ...
              >
                <div class="swiper-slide">
                  <div
                    @click.prevent="changeAddon(addon)"
                  ...
                  >
                    ...
                    <div class="rounded-r-lg w-full py-1 px-2">
                      ...
                      <p
                        v-if="addon.variation_names.length > 0"
                  ...
                  >
                    <button
                      ...
                      data-modal="#item-info-modal"
                      @click.prevent="
                        infoModalShow(addon.addon_item_name, addon.caution)
                      "
                    >
                      <i
                        class="lab lab-information font-fill-paragraph transition lab-font-size-16"
                    ...
                    <div class="flex items-center indec-group">
                      <button
                        @click.prevent="addonQuantityDecrement(addon.id)"
                      ...
                      />
                      <button
                        @click.prevent="addonQuantityIncrement(addon.id)"
        ...
        <div class="mb-6">
          <h3
            class="text-xs leading-6 font-medium capitalize mb-2 text-heading"
          >
...
export default {
  ...
  methods: {
    ...
    variationModalShow: function (selectedItem) {
      this.$store
        .dispatch("frontendItem/details", selectedItem.id)
        .then((res) => {
          ...
          if (this.item.itemAttributes.length > 0) {
            _.forEach(this.item.itemAttributes, (element) => {
              if (typeof this.item.variations[element.id][0] !== "undefined") {
                this.temp.item_variations.variations[
                  this.item.variations[element.id][0].item_attribute_id
                ] = this.item.variations[element.id][0].id;
                this.temp.item_variations.names[element.name] =
                  this.item.variations[element.id][0].name;
                this.temp.item_variation_total +=
                  this.item.variations[element.id][0].convert_price;
              }
            });
          }
          ...
          this.temp.convert_price =
            item.offer.length > 0
              ? item.offer[0].convert_price
              : item.convert_price;
          this.temp.currency_price =
            item.offer.length > 0
              ? item.offer[0].currency_price
              : item.currency_price;
          this.temp.total_price =
            (item.offer.length > 0
              ? item.offer[0].convert_price
              : item.convert_price) + this.temp.item_variation_total;
    ...
    totalPriceSetup: function () {
      ...
      this.temp.total_price = parseFloat(
        ((this.item.offer.length > 0
          ? this.item.offer[0].convert_price
          : this.item.convert_price) +
          this.temp.item_variation_total +
          this.temp.item_extra_total) *
          this.temp.quantity +
          item_addon_total
      );
    },
    ...
    changeAddon: function (addon) {
      if (typeof this.addons[addon.id] === "undefined") {
        this.addons[addon.id] = {
          ...
          discount: 0,
          currency_price:
            addon.offer.length > 0
              ? addon.offer[0].currency_price
              : addon.addon_item_currency_price,
          convert_price:
            addon.offer.length > 0
              ? addon.offer[0].convert_price
        ...
        if (
          addon.variations !== "undefined" &&
          Object.keys(addon.variations).length !== 0
        ) {
          _.forEach(addon.variations, (variationId, attributeId) => {
            this.addons[addon.id].item_variations.variations[attributeId] =
              variationId;
          });
        }
        if (addon.variation_names.length > 0) {
          _.forEach(addon.variation_names, (variation) => {
            this.addons[addon.id].item_variations.names[
    ...
    addToCart: function () {
      ...
      if (this.itemArrays.length > 0) {
        this.$store
          .dispatch("frontendCart/lists", this.itemArrays)
          .then((res) => {
            this.item = null;
            this.temp.name = "";
            this.temp.image = "";
