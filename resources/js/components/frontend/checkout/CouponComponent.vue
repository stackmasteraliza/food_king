<template>
  <div
    v-if="Object.keys(cartCoupon).length !== 0"
  ...
  >
    ...
    >
      ...
      <span
        class="capitalize break-words tracking-wide text-[10px] leading-4"
        >{{ $t("label.offer_applied") }}</span
      >
    </div>
    <div class="w-full ltr:rounded-l-lg rtl:rounded-r-lg relative bg-white">
      ...
      <p class="text-[10px] first-letter:capitalize leading-[14px]">
        {{
          $t("message.you_saved", {
            amount: cartCoupon.currency_discount,
          })
        }}
      </p>
      <button
        ...
        type="button"
        class="lab lab-delete-bold text-sm absolute top-1/2 ltr:right-5 rtl:left-5 -translate-y-1/2 text-[#FB4E4E]"
      ></button>
    </div>
  </div>
  ...
  <button
    ...
    data-modal="#coupon-modal"
    type="button"
  >
    <i class="lab-ticket-discount lab-font-size-30 text-3xl text-[#008BBA]"></i>
    <span class="text-xs font-medium text-left flex-auto"
      >{{ $t("label.select_apply")
      }}<small class="block text-[9px]">{{
        $t("message.get_discount")
      }}</small></span
    >
    <i
      class="lab-arrow-right rtl:rotate-180 lab-font-bold lab-font-size-22 text-lg text-[#008BBA]"
    ></i>
  </button>
      ...
      <div class="modal-body">
        ...
        >
          <div class="p-2 border-b border-[#EFF0F6]">
            <h3
              class="py-1 px-2 rounded font-medium text-xs w-fit mb-2 bg-[#FFDB1F]"
            >
              {{ $t("label.code") }}: {{ coupon.code }}
            </h3>
            <h4 style="font-family: 'saudi_riyal'" class="text-xs">
              {{
                $t("message.discount_off", {
                  discount:
                    coupon.discount_type === taxTypeEnum.PERCENTAGE
          ...
          <button
            @click.prevent="appCouponButton(coupon)"
            type="button"
          ...
          <div v-if="coupon.description">
            <button
              @click.prevent="termStatus(coupon.id)"
              type="button"
            ...
            <div
              :id="'terms-' + coupon.id"
              class="h-0 overflow-hidden transition"
            >
...
export default {
  ...
  methods: {
    ...
    couponChecking() {
      this.$store
        .dispatch("frontendCoupon/checking", {
    ...
    destroyCoupon() {
      this.$store
        .dispatch("frontendCart/destroyCoupon")
        .then((res) => {
          this.coupon({});
