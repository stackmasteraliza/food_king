...
class ItemService
{
    ...
    public function list(PaginateRequest $request)
    {
        try {
            ...
            return Item::with('media', 'category', 'tax')->withCount('orders')->where(function ($query) use ($requests) {
    ...
    public function simpleList(PaginateRequest $request)
    {
        try {
            ...
            return Item::with('media', 'category', 'offer')->withCount('orders')->where(function ($query) use ($requests) {
                foreach ($requests as $key => $request) {
                    if (in_array($key, $this->itemFilter)) {
                        if ($key == "except") {
                            $explodes = explode('|', $request);
